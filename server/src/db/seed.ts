...
const seed = async () => {
  ...
  try {
    ...
    await client.query("INSERT INTO users (full_name, email, password, role) VALUES ($1, $2, $3, $4)", [
    ...
    await client.query("INSERT INTO users (full_name, email, password, role) VALUES ($1, $2, $3, $4)", [
      process.env.STAFF_FULL_NAME,
      process.env.STAFF_EMAIL,
      await bcrypt.hash(process.env.STAFF_PASSWORD!, 10),
      "staff"
    ]);
